import {SYSTEMS, type Atlas, type Concept, type Part, type SystemId} from './anatomy.ts';
...
export const SYSTEM_VI:Record<SystemId,{name:string;description:string}> = {
 ...
 adipose:{name:'Mô mỡ',description:'Mô mỡ dự trữ năng lượng, đệm và bảo vệ cấu trúc. Mỡ dưới da và mạc nối nằm ở những lớp giải phẫu khác nhau.'},
...
export function conceptSystem(c:Concept,parts:Map<string,Part>):SystemId{
 ...
 if(/^(subcutaneous abdominal adipose tissue|omentum)/.test(n))return 'adipose';
