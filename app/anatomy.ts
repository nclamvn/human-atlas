export type SystemId = 'skeletal'|'muscular'|'arterial'|'venous'|'nervous'|'digestive'|'respiratory'|'urinary'|'reproductive'|'lymphatic'|'endocrine'|'integumentary'|'adipose'|'connective'|'sensory'|'cardiac'|'pregnancy';
export const SYSTEMS: {id:SystemId;name:string;color:string;description:string}[] = [
 ...
 {id:'adipose',name:'Adipose tissue',color:'#d8b678',description:'Adipose tissue stores energy, cushions structures, and participates in metabolism. Subcutaneous fat and the omentum occupy different anatomical layers.'},
...
export interface SceneState {inspectorOpen?:boolean;explode:number;visible:SystemId[];selected:string[];isolate:boolean;view:View;rotate:boolean;reset:number;lens?:'solid'|'xray';lesson?:string;phase?:number;narrationPlaying?:boolean;tourEffect?:'none'|'study-flow'|'blood-return'|'blood-right-heart'|'blood-to-lungs'|'blood-oxygenate'|'blood-left-heart'|'blood-aorta'|'blood-systemic'|'blood-cycle';focusAperture?:boolean;cameraDistanceScale?:number;cameraDirection?:[number,number,number]}
